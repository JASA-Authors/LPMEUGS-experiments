...
Source_Table7=function(repli=5,n=200,p=160,verbose=T,save_result=T){
  ## Input:
  ...
  if(n<p){
    ...
    for (i in 1:(p-1)) {
      for (j in (i+1):p) {
        ...
        Sigmatrue5[i,j]=Sigmatrue5[i,j]+r
        Sigmatrue5[j,i]=Sigmatrue5[j,i]+r
      }
    }
    ...
    x <- foreach(i=1:repli,.combine='rbind') %dopar% compare_par(i, Sigmatrue5, n, p)
  ...
  if(verbose==TRUE){
    print(paste("Results for Table 7 with n =",n,"p =",p))
    print(result)
  }
  
  if(save_result==TRUE){
    write.csv(result,paste("./Table7/n",n,"_p", p, ".csv",sep=""))
  }
