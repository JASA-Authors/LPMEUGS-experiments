...
Source_Table6=function(repli=5,n=200,p=160,verbose=T,save_result=T){
  ## Input:
  ...
  if(n<p){
    ...
    compare_par=function(i, Sigmatrue, n, p){
      ...
      error6=sqrt(sum((solve(Sigmatrue)-flare::sugm(X,nlambda=1,method = "tiger")$icov[[1]])^2))
      error7=sqrt(sum((solve(Sigmatrue)-flare::sugm(X,nlambda=1,method = "clime")$icov[[1]])^2))
      error8=sqrt(sum((solve(Sigmatrue)-POET::POET(t(X))$SigmaY)^2))
  ...
  if(verbose==TRUE){
    print(paste("Results for Table 6 with n =",n,"p =",p))
    print(result)
  }
  
  if(save_result==TRUE){
    write.csv(result,paste("./Table6/n",n,"_p", p, ".csv",sep=""))
  }
