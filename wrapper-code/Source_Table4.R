...
Source_Table4=function(repli=5,n=200,p=160,verbose=T,save_result=T){
  ## Input:
  ...
  if(n<p){
    ...
    Sigmatrue2=kronecker(diag(K),ar1_cor(p/K,0.9)) # true covariance matrix
    ...
    x <- foreach(i=1:repli,.combine='rbind') %dopar% compare_par(i, Sigmatrue2, n, p)
  ...
  if(verbose==TRUE){
    print(paste("Results for Table 4 with n =",n,"p =",p))
    print(result)
  }
  
  if(save_result==TRUE){
    write.csv(result,paste("./Table4/n",n,"_p", p, ".csv",sep=""))
  }
