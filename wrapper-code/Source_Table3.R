...
Source_Table3=function(repli=5,n=200,p=160,verbose=T,save_result=T){
  ## Input:
  ...
  if(n<p){
    ...
    x <- foreach(i=1:repli,.combine='rbind') %dopar% compare_par(i, Sigmatrue1, n, p)
  ...
  if(verbose==TRUE){
    print(paste("Results for Table 3 with n =",n,"p =",p))
    print(result)
  }
  
  if(save_result==TRUE){
    write.csv(result,paste("./Table3/n",n,"_p", p, ".csv",sep=""))
  }
