...
Source_Table8=function(repli=5,verbose=T,save_result=T){
  ## Input:
  ...
  result1=foreach(i=1:repli,.combine='rbind') %dopar% MDDA(i)
  ...
  rownames(result1)[(nrow(result1)-1):nrow(result1)]=c("mean","sd")
  
  if(verbose==TRUE){
    print("Results for Table 8")
    print(result1)
  }
  
  if(save_result==TRUE){
    write.csv(result1,paste("./Table8/Table8",".csv",sep=""))
  }
